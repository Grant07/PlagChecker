...
def profile(request):
    ...
    try:
        ...
        try:
            ...
            admin_object = Admin.objects.get(user_id=request.user.id)
    ...
    if student_object:
        data = {
            ...
            "yos": (
                student_object.year_of_study if student_object.year_of_study else None
            ),
        ...
        data = {
            "department": (
                lecturer_object.department.name if lecturer_object.department else None
            ),
...
def update_profile(request):
    try:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    admin_object = Admin.objects.create(
                        user_id=request.user.id, employee_id=request.user.username
                    )

        # Handle the case when no user object is found
        if not (student_object or lecturer_object or admin_object):
        ...
        if "image__upload-btn" in request.FILES:
            ...
            user = student_object.user if student_object else (lecturer_object.user if lecturer_object else admin_object.user)
            user.image = image_data

        # Update user information
        user = student_object.user if student_object else (lecturer_object.user if lecturer_object else (admin_object.user if admin_object else None))
        user.phone = phone
        user.email = email
        if admin_object:
...
def change_password(request):
    try:
        ...
        if new_password != confirm_password:
            return JsonResponse(
                {"message": "New password and confirm password do not match. 🙅🏽‍♂️"},
                status=400,
            )

        if not request.user.check_password(current_password):
            return JsonResponse(
                {"message": "Incorrect Current Password. ❌"}, status=400
            )
...
def change_preferences(request):
    try:
        try:
            ...
            preferences = Preferences.objects.create(user=request.user)
        ...
        if dark_mode not in ["true", "false"] or notifications not in ["true", "false"]:
            return JsonResponse(
                {"message": "Invalid input for preferences."}, status=400
            )
        ...
        return JsonResponse(
            {"message": "Preferences changed successfully."}, status=200
        )

    except Preferences.DoesNotExist:
        return JsonResponse(
            {"message": "Preferences not found for this user."}, status=404
        )
